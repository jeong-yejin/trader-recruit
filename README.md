...
**현재 프론트엔드만 구현되어 있습니다. 서버 측은 없습니다.**

우선순위 순.

| # | 작업 | 현재 | 필요한 것 |
|---|---|---|---|
| 1 | **지원서 제출 연동** | `CONFIG.formEndpoint` 가 비어 있어 제출 데이터가 전송되지 않습니다. 폼은 완료 화면만 띄웁니다 | 엔드포인트 주소 1개. 전송 로직은 구현되어 있어 값만 채우면 동작합니다 |
| 2 | **지원서 저장소** | 없음 | DB 또는 시트. 필드 14개 + 제출 시각 + 이벤트/로케일 |
| 3 | **슬랙 알림봇** | 없음 | 신규 지원 시 채널 알림. 이름 · 핸들 · 거래소 · 트랙레코드 링크면 심사 시작 가능 |
| 4 | **약관 · 개인정보처리방침** | 동의 체크박스는 필수인데 링크할 문서가 없습니다 | 개인정보 항목을 수집하므로 처리방침 문서와 링크가 필요합니다 |
| 5 | **서버 측 중복 차단** | `localStorage` 기반이라 다른 브라우저 · 시크릿 창 · 데이터 삭제로 우회됩니다 | 이메일 유니크 체크를 서버로 이동 |
| 6 | **지원자 확인 메일** | 없음. 완료 화면이 유일한 확인 수단이라 새로고침하면 사라집니다 | 트랜잭셔널 메일 1통 |
| 7 | **스팸 · 봇 차단** | 없음 | 레이트 리밋 또는 캡차 |
| 8 | **좌석 상태 연동** | `CONFIG.seatsOpen` 하드코딩. 사람이 직접 수정 | 심사 상태와 연결 |
| 9 | **분석** | 없음 | CTA 클릭 · 폼 시작 · 폼 이탈 · 제출 이벤트 |

### 조건 분기가 필요한 지점

지금은 전부 한 가지 상태만 그립니다.

| 조건 | 지금 | 필요한 분기 | 위치 |
|---|---|---|---|
| 좌석 마감 (`seatsOpen === 0`) | 좌석 그리드만 `taken` 으로 바뀌고 **폼은 열려 있습니다** | 폼 닫기 + 마감 카피 + CTA 비활성화 | `SeatBand`, `ApplyForm`, `StickyCta`, `FinalCta` |
| 이벤트 종료 (`Date.now() > eventDate`) | 카운트다운이 `00:00:00:00` 을 계속 표시 | 종료 카피 + 다음 이벤트 안내 | `Countdown.tsx` — `remaining()` 이 `diff <= 0` 을 이미 계산하므로 반환값만 분기 |
| 재방문 지원자 | 빈 폼을 처음처럼 표시. 제출해야 중복을 알려줌 | 마운트 시점 확인 → 안내 배너 | `ApplyForm.tsx` |
| 제출 실패 | 안내 배너만. 입력값은 폼에 남음 | 재시도 버튼 | `ApplyForm.tsx` |
| 이벤트별 상태 | `seatsOpen` · `formEndpoint` 이 두 이벤트 공용 | `Record<EventSlug, ...>` 로 분리 | `lib/config.ts` — `eventDate` 가 이미 이 형태 |
